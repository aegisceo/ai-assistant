    ...
    updateConnectionStatus(service, status) {
        ...
        if (indicator && statusText) {
            if (status.connected) {
                ...
                // Update connection buttons for Gmail
                if (service === 'gmail') {
                    this.updateConnectionButtons(true, status.email);
                }
            } else {
        ...
        if (connectionInfo) {
            if (status.email) {
                connectionInfo.textContent = `Connected to ${status.email}`;
                connectionInfo.className = 'text-gray-600 mt-1 connection-info';
            } else {
                // Show demo mode context
                const app = window.neuroFlowApp;
                if (app && app.demoMode && service === 'gmail') {
                    connectionInfo.textContent = 'Demo mode - Backend server not available';
                    connectionInfo.className = 'text-blue-600 mt-1 connection-info';
                } else {
                    connectionInfo.textContent = service === 'gmail' ? 'Not connected' : 'Requires Gmail connection';
                    connectionInfo.className = 'text-gray-500 mt-1 connection-info';
                }
            }
        }
    }
